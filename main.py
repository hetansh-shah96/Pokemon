...
import theme
from theme import (
    WIDTH, HEIGHT, FPS, BG, BG_ALT, PANEL, PANEL_LIGHT, PANEL_EDGE, TEXT,
...
class Game:
    ...
    def present(self):
        ...
        scale = max(0.01, min(win_w / WIDTH, win_h / HEIGHT))
    ...
    def draw_title(self):
        ...
        for i in range(6):
            pygame.draw.rect(self.screen, (20 + i * 2, 26 + i * 2, 20 + i * 2),
                              (0, i * (HEIGHT // 6), WIDTH, HEIGHT // 6))
        draw_text(self.screen, "POKEMON", (WIDTH // 2, HEIGHT // 2 - 110), size=70,
                  color=ACCENT, bold=True, center=True, shadow=True)
        draw_text(self.screen, "INDIGO LEAGUE CHALLENGE", (WIDTH // 2, HEIGHT // 2 - 40),
                  size=34, color=TEXT, bold=True, center=True, shadow=True)
        draw_text(self.screen, "Draft a team of 6. Choose your arena.", (WIDTH // 2, HEIGHT // 2 + 30),
                  size=20, color=TEXT_DIM, center=True)
        draw_text(self.screen, "Face the Legendary Six -- and try to survive.", (WIDTH // 2, HEIGHT // 2 + 58),
                  size=20, color=TEXT_DIM, center=True)
        if (pygame.time.get_ticks() // 500) % 2 == 0:
            draw_text(self.screen, "PRESS ENTER OR CLICK TO START", (WIDTH // 2, HEIGHT - 90),
    ...
    def builder_layout(self):
        ...
        cols, rows = 5, 2
        margin = 14
        card_w = (WIDTH - self.SIDEBAR_W - margin * (cols + 1)) // cols
        card_h = 150
        top = 70
    ...
    def builder_buttons(self):
        reroll = Button((18, HEIGHT - 66, 220, 46), "REROLL (R)")
        ready = len(self.team_species) == TEAM_SIZE
        confirm = Button((WIDTH - self.SIDEBAR_W + 8, HEIGHT - 66, self.SIDEBAR_W - 26, 46),
    ...
    def draw_builder(self):
        ...
        if spinning:
            header = "Rerolling..."
        elif self.must_reroll:
            header = "Pick locked in! Reroll to see your next choice."
        else:
            header = f"{len(self.team_species)}/{TEAM_SIZE} chosen -- click a card to draft it"
        header_color = ACCENT if self.must_reroll and not spinning else TEXT_DIM
        draw_text(self.screen, header, (18, 48), size=16, color=header_color, bold=self.must_reroll and not spinning)
        ...
        # team panel
        panel_rect = pygame.Rect(WIDTH - self.SIDEBAR_W, 40, self.SIDEBAR_W - 18, TEAM_SIZE * 66 + 20)
        ...
        detail_rect = pygame.Rect(18, 406, WIDTH - self.SIDEBAR_W - 36, HEIGHT - 406 - 82)
    ...
    def handle_arena(self, event):
        ...
        back = Button((18, HEIGHT - 66, 200, 46), "BACK")
        go = Button((WIDTH - 270, HEIGHT - 66, 250, 46), "ENTER BATTLE!", enabled=self.arena_choice is not None)
        if event.type == pygame.KEYDOWN:
    ...
    def draw_arena(self):
        ...
        panel = pygame.Rect(24, 450, WIDTH - 48, 150)
        draw_panel(self.screen, panel, bg=BG_ALT)
        draw_text(self.screen, "YOUR OPPONENTS -- THE LEGENDARY SIX",
                  (panel.x + 14, panel.y + 10), size=16, color=ACCENT, bold=True)
        x = panel.x + 14
        y = panel.y + 40
        for sp in LEGENDARY_SPECIES:
            icon = sprites.get_battle_sprite(sp, size_px=64)
            self.screen.blit(icon, (x, y))
            draw_text(self.screen, sp.name, (x, y + 66), size=12, color=TEXT, center=False)
            x += 96
        ...
        if self.battle_menu == "root":
            ...
            for i, mon, rect in self.party_buttons():
                if rect.collidepoint(pos) and not mon.fainted and i != b.player_active_idx:
                    ...
                    self.anim_queue = list(b.turn_events)
                    self.battle_menu = "root"
            back = pygame.Rect(WIDTH - 300, HEIGHT - 44, 270, 34)
    ...
    def draw_battle(self):
        ...
        pygame.draw.rect(self.screen, th["panel"], (0, HEIGHT - 260, WIDTH, 260))

        cpu_mon = self.display_cpu_mon or b.cpu_active
        player_mon = self.display_player_mon or b.player_active
        self.draw_battler_sprite(cpu_mon, (WIDTH - 220, 60), 140, "cpu")
        self.draw_battler_sprite(player_mon, (70, HEIGHT - 470), 170, "player")

        self.draw_hud(cpu_mon, pygame.Rect(30, 40, 320, 90), is_player=False)
        self.draw_hud(player_mon, pygame.Rect(WIDTH - 400, HEIGHT - 360, 340, 100), is_player=True)

        # log panel
        log_rect = pygame.Rect(20, HEIGHT - 240, WIDTH - 340, 220)
        ...
        menu_rect = pygame.Rect(WIDTH - 320, HEIGHT - 240, 300, 220)
    ...
    def handle_result(self, event):
        again = Button((WIDTH // 2 - 260, HEIGHT - 130, 240, 56), "PLAY AGAIN")
        quit_btn = Button((WIDTH // 2 + 20, HEIGHT - 130, 240, 56), "QUIT")
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
    ...
    def draw_result(self):
        ...
        draw_text(self.screen, title, (WIDTH // 2, 160), size=42, color=ACCENT if won else BAD,
        ...
        draw_text(self.screen, sub, (WIDTH // 2, 220), size=18, color=TEXT, center=True)

        p_ko = sum(1 for m in b.player_team if m.fainted)
        c_ko = sum(1 for m in b.cpu_team if m.fainted)
        draw_text(self.screen, f"Your Pokemon fainted: {p_ko}/6", (WIDTH // 2, 280), size=16, color=TEXT_DIM, center=True)
        draw_text(self.screen, f"Legendary Six fainted: {c_ko}/6", (WIDTH // 2, 306), size=16, color=TEXT_DIM, center=True)
